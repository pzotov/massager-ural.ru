...
function getChildren( $sub ){
	global $db;
	if( $children = $db->get_col( "SELECT Subdivision_ID FROM Subdivision WHERE Parent_Sub_ID={$sub} AND Checked=1" ) ){
		foreach( $children as $c ){
			$children = array_merge( $children, getChildren($c) );
		}
	} else $children = array();
	return $children;
}
...
function firstSentence( $text, $maxlen = 100, $tobecon = '&hellip;' ){
